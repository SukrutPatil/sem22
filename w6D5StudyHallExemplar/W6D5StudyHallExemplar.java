...
public class W6D5StudyHallExemplar {
	 ...
	 * diagram into an executable Java program using the oddNumber Recognizer. </p>
	 * 
	...
	private static String executeFSM(String input) {
		...
		case 1:
			// States 3 is a not final state, so we must be sure there are no more characters.
						if (currentCharNdx<=input.length())
		...
		case 3:
			// State 1 is final state, so we can display a very specific error message.
