...
public class MeasuredValueRecognizer {
	 ...
	 * @version 1.00        2018-02-20 Complete code with stubs replaced.
	 * 
	 */
	...
	public static String checkMeasureValue(String input) {
		...
		while (running) {
			...
			case 4: 
				...
				// The execution of this state is finished
				break;
// Checking FSM diagram find the outbonding arrors and code arrording to it.
			case 5: 
				//State 5 has two valid transitions. Each is addressed by an if statement
				
				//This  is not a final state
				finalState = false;
				//If value is in between 0-9 it moves to stage 7
				if (currentChar >='0' && currentChar <='9') {
					nextState = 7;
					break;
				}
                // If values are +/- it moves to state state 6
